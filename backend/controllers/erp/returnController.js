...
const generateReturnNo = async (branchId) => {
  ...
  const last   = await Return.findOne({
    where: { return_no: { [Op.like]: `${prefix}${ymd}%` } },
    order: [['id','DESC']],
  });
...
const createReturn = async (req, res, next) => {
  ...
  try {
    ...
    // items: [{ order_item_id, qty_return }]

    if (!items?.length) {
      await t.rollback();
      return res.status(400).json({ success: false, message: 'Minimal 1 item retur' });
    }
    ...
    for (const item of items) {
      ...
      if (item.qty_return > orderItem.qty) throw new Error(`Qty retur melebihi qty order untuk ${orderItem.product_name}`);
      ...
      returnItems.push({
        ...
        sell_price:    toNum(orderItem.sell_price),
        subtotal,
      });
    }
    ...
    return res.status(201).json({
      success: true,
      message: `Retur ${returnNo} berhasil dibuat`,
      data: { return: ret },
    });
...
const confirmReturn = async (req, res, next) => {
  ...
  try {
    ...
    if (ret.status !== 'pending') {
      await t.rollback();
      return res.status(400).json({ success: false, message: `Retur sudah ${ret.status}` });
    }

    const order = await Order.findByPk(ret.order_id, { transaction: t });

    // Restock if needed
    if (ret.restock) {
      for (const item of ret.items) {
        let stock = await Stock.findOne({
          where: { product_id: item.product_id, branch_id: ret.branch_id },
          transaction: t,
        });
        if (!stock) {
          stock = await Stock.create({
            product_id: item.product_id, branch_id: ret.branch_id, qty: 0,
          }, { transaction: t });
        }
        ...
        await StockMovement.create({
          ...
          type: 'in', qty: item.qty_return,
          qty_before: qtyBefore, qty_after: qtyBefore + item.qty_return,
          ref_type: 'return', ref_id: ret.id,
          notes: `Retur ${ret.return_no} — ${item.product_name}`,
          created_by: req.user?.id,
        }, { transaction: t });
      }
    }
    ...
    return res.json({
      success: true,
      message: `Retur ${ret.return_no} dikonfirmasi${ret.restock ? ' — stok bertambah' : ''}`,
      data: { return: ret },
    });
  } catch (err) { await t.rollback(); next(err); }
};
