...
import {
  ...
  AlertTriangle, Loader2, Database, Calendar, Play,
  HardDrive, FileJson,
} from 'lucide-react';
...
export default function BackupErpPage() {
  ...
  const [schedForm, setSchedForm] = useState({ frequency:'daily', hour:3, enabled:true });
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast.error('Gagal memuat data backup ERP'); }
  ...
  const handleBackup = async () => {
    setRunning(true);
    const toastId = toast.loading('Membuat backup ERP... (bisa beberapa menit)');
    try {
      const r = await api.post('/backup-erp/run');
  ...
  const handleRestore = async () => {
    ...
    const toastId = toast.loading('Memulihkan data ERP...');
    try {
      const r = await api.post('/backup-erp/restore', {
  ...
  const handleDeleteBackup = async (id) => {
    ...
    try {
      await api.delete(`/backup-erp/${id}`);
  ...
  const handleSaveSchedule = async () => {
    setSavingSched(true);
    try {
      await api.put('/backup-erp/schedule', schedForm);
      toast.success('Jadwal backup ERP disimpan');
      load();
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">💾 Backup & Restore ERP</h1>
          <p className="page-subtitle">Backup otomatis dan manual seluruh data ERP & Toko Online</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="table-wrapper p-5 space-y-4">
            <div className="flex items-center gap-2 pb-3 border-b border-[var(--border)]">
              ...
              <p className="font-bold text-sm">Auto Backup ERP</p>
            ...
            <div>
              ...
              <p className="text-[11px] text-[var(--text-muted)] mt-1">
                💡 Disarankan jam 03:00 (dini hari) — berbeda 1 jam dari backup HRD agar tidak bersamaan
              </p>
            </div>
          ...
          <div className="table-wrapper overflow-hidden">
            <div className="px-4 py-3 border-b border-[var(--border)] flex items-center justify-between">
              <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">
                Riwayat Backup ERP ({backups.length}/{30} maks)
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                <p className="font-semibold text-[var(--text-primary)]">Belum ada backup ERP</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[var(--bg-card)] rounded-2xl border-2 border-amber-400 shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-black text-base text-amber-700">Konfirmasi Restore ERP</p>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 space-y-1">
              ...
              <p>Restore akan menimpa data ERP yang ada dengan data dari backup ini ({fmtDate(confirmRestore.created_at)}).</p>
              <p>Data HRD/Karyawan <strong>tidak akan diubah</strong> — backup ini khusus data ERP & Toko Online.</p>
            </div>
