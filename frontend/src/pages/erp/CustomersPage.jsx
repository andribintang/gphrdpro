    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      <div className="modal-box max-w-md" onClick={e=>e.stopPropagation()}>
        <div className="modal-header">
          <h3 className="text-sm font-bold">{isEdit ? 'Edit Pelanggan' : 'Tambah Pelanggan'}</h3>
        ...
        <div className="modal-body">
          {[
            ...
            <div key={f.k}>
              ...
              <input type={f.t||'text'} value={form[f.k]} onChange={e=>sf(f.k,e.target.value)} className="input-base" />
            </div>
          ))}
          <div>
            <label className="field-label">Catatan</label>
            <textarea value={form.notes} onChange={e=>sf('notes',e.target.value)} rows={2} className="input-base resize-none" />
          </div>
        </div>
...
export default function CustomersPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const res = await erpService.getCustomers({ limit:500 });
  ...
  const columns = [
    ...
    {
      key:'phone', label:'No. HP', nowrap:true,
      render: (v) => v
        ? <span className="flex items-center gap-1.5 text-[var(--text-secondary)]"><Phone size={12} />{v}</span>
        : <span className="text-[var(--text-muted)]">—</span>,
    },
    {
      key:'city', label:'Kota',
      render: (v) => v
        ? <span className="flex items-center gap-1.5 text-[var(--text-secondary)]"><MapPin size={12} />{v}</span>
        : <span className="text-[var(--text-muted)]">—</span>,
    },
    {
      key:'total_orders', label:'Order', sortable:true, align:'center', nowrap:true,
      render: (v) => <span className="font-semibold text-[var(--text-primary)]">{v || 0}x</span>,
    },
    {
  ...
  return (
    <div className="section animate-fade-in">
      <nav className="flex items-center gap-1.5 mb-5 text-xs text-[var(--text-muted)] select-none">
        ...
        <span className="font-semibold text-[var(--text-primary)]">Pelanggan</span>
      </nav>
      <div className="page-header">
        <div>
          <h1 className="page-title">Pelanggan</h1>
          <p className="body-sm text-[var(--text-muted)]">{customers.length} pelanggan terdaftar</p>
        </div>
      ...
      <DataTable
        ...
        searchPlaceholder="Cari nama, HP, email..."
