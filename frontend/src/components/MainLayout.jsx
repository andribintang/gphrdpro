      ...
      <header className="sticky top-0 z-40 safe-top glass border-b border-[var(--border)]">
        <div className="flex items-center justify-between h-14 px-4 max-w-screen-xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={toggleTheme} className="w-9 h-9 rounded-xl border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)] transition-all">
              {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
            <button onClick={() => setSidebarOpen(true)} className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl border border-[var(--border)] hover:bg-[var(--bg-secondary)] transition-all">
          ...
          <aside className="relative w-72 h-full bg-[var(--bg-card)] border-l border-[var(--border)] animate-slide-down flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 border-b border-[var(--border)]">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center shadow-glow flex-shrink-0">
            ...
            <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
              ...
              {/* Reports for admin/hr/supervisor */}
              {['admin','hr','supervisor'].includes(user?.role) && (
            ...
            <div className="p-3 border-t border-[var(--border)] space-y-1">
              ...
              <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-red-950 transition-all">
          ...
          {visibleNav.slice(0, 5).map(item => (
            <NavLink key={item.to} to={item.to}
              className={({ isActive }) =>
                ...
                ${isActive ? 'text-brand-500 dark:text-brand-400' : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)]'}`
              }>
              {({ isActive }) => (
                <>
