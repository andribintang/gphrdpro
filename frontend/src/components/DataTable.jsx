...
const SortIcon = ({ col, sortCol, sortDir }) => {
  ...
  return sortDir === 'asc'
    ? <ChevronUp size={13} className="text-[var(--brand-600)]" />
    : <ChevronDown size={13} className="text-[var(--brand-600)]" />;
};
      ...
      <div className="flex items-center gap-2 p-4 border-b border-[var(--border)] flex-wrap">
          ...
          <div className="relative flex-1 min-w-48">
            ...
            {search && (
              <button onClick={() => handleSearch('')}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 btn-icon-sm">
                <X size={13} />
              </button>
            )}
          </div>
        ...
        {filters.length > 0 && (
          <button onClick={() => setShowFilters(v => !v)}
            className={`btn-secondary h-9 px-3 text-sm gap-1.5 ${showFilters ? 'bg-[var(--bg-tertiary)]' : ''}`}>
            <SlidersHorizontal size={14} />
            Filter
            {activeFilterCount > 0 && (
              <span className="w-5 h-5 rounded-full bg-[var(--brand-600)] text-white text-[10px] font-bold flex items-center justify-center">
                {activeFilterCount}
              </span>
            )}
          </button>
          ...
          {filters.map(f => (
            <div key={f.key} className="flex items-center gap-2">
              ...
              <select
                ...
                className="input-base h-8 text-xs pr-8 min-w-28">
            ...
            <tr className={`border-b border-[var(--border)] bg-[var(--bg-secondary)] ${stickyHeader ? 'sticky top-0 z-10' : ''}`}>
              {columns.map(col => (
                <th key={col.key}
                  ...
                  className={`px-5 py-3.5 text-left text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]
              ...
              {actions && (
                <th className="px-5 py-3.5 text-right text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)] w-24">
                  Aksi
                </th>
              )}
            </tr>
