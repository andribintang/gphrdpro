...
const Pagination = ({ page, totalPages, total, pageSize, onPage }) => {
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3.5 border-t border-[var(--border)] bg-[var(--bg-card)]">
    ...
    `}>
      ...
      <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-[var(--border)] flex-wrap bg-[var(--bg-card)]">
        ...
        <div className="ml-auto">
          <span className="text-[12px] text-[var(--text-muted)]">
            <span className="font-semibold text-[var(--text-secondary)]">{search ? filtered.length : data.length}</span>
            {search ? ' hasil' : ' data'}
          </span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto scrollbar-thin">
        <table className="w-full text-sm border-collapse" style={{minWidth:'560px'}}>
          ...
          <tbody>
            {loading ? (
              Array.from({length:6}).map((_,i)=><SkeletonRow key={i} cols={columns.length+(actions?1:0)}/>)
            ) : paginated.length===0 ? (
              ...
              paginated.map((row,idx)=>{
                ...
                return (
                  <tr key={key} onClick={()=>onRowClick?.(row)}
                    ...
                    className={`border-b border-[var(--border-subtle)] last:border-0 transition-colors duration-100 group ${onRowClick?'cursor-pointer':''} ${zebra&&idx%2===0?'bg-[var(--bg-secondary)]/25':''} ${isHovered?'bg-[var(--brand-600)]/[0.035] dark:bg-[var(--brand-600)]/[0.07]':''}`}>
                    {columns.map(col=>(
                      <td key={col.key}
      ...
      {!loading&&<Pagination page={page} totalPages={totalPages} total={filtered.length} pageSize={pageSize} onPage={setPage}/>}
    </div>
  );
}
